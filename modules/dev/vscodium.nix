...
# 2. Branding Spoof (Fixes assertVSDA)
data['quality'] = 'stable'
data['commit'] = '${unstable.vscode.version}'
data['nameShort'] = 'Code'
data['nameLong'] = 'Visual Studio Code'
data['applicationName'] = 'code'
data['dataFolderName'] = '.vscode' # Mimics official folder name
data['win32AppUserModelId'] = 'Microsoft.VisualStudioCode'
data['urlProtocol'] = 'vscode'
...
EOF
      ...
      # Physically remove auth blocks that might conflict with spoofing
  ...
  environment.systemPackages = [
    ...
    # Create an alias so the system thinks 'code' is running
    (pkgs.writeShellScriptBin "code" "exec ${pkgs.vscodium-isolated}/bin/codium \"$@\"")
